...
float BPF::Output(float pTime, float pInput) {
	...
	float inbuf1 =  mGainB0 * pInput;
	float integrate = mInputUnitDelay1.PushAndGetVal(pInput);
	float integrate2 = mInputUnitDelay2.PushAndGetVal(integrate);
	float inbuf2 = integrate2 * mGainB2;
	float buf = inbuf1 - inbuf2;
